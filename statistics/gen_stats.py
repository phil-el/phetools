...
if __name__ == "__main__":
    ...
    if opt_write:
        ...
        if opt_diff:
            print("date: Difference between " + ' '.join(time.ctime(old_time).split()[:3]) + " " +
                  time.ctime(old_time).split()[4] + " and " + ' '.join(time.ctime(stats_time).split()[:3]) + ' ' +
                  time.ctime(stats_time).split()[4])
        else:
