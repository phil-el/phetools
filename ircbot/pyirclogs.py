...
def handleTopic(connection,event):
    _real_handler(event.source().split( '!' )[0] + ' a changé le topic en : "' + event.arguments()[0],name=event.target())
...
def handleMode ( connection, event ):
   ...
   if len ( event.arguments() ) < 2:
      _real_handler(event.source().split ( '!' ) [ 0 ] + ' a changé le mode du canal : ' + event.arguments() [ 0 ],name=event.target())

   # User mode
   else:
      _real_handler(event.source().split ( '!' ) [ 0 ] + ' a changé le mode de : '+ ' '.join(event.arguments() [ 1: ]) + ' en : ' + event.arguments()[ 0 ],name=event.target())
