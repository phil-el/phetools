...
def do_match(target, filename, djvuname, number, verbose=False, prefix="Page"):
	...
	if is_poem:
            ...
            if no_color[-3:]==u"\n\n\xab": no_color = no_color[:-3]
	else:
            no_color = no_color.rstrip('\n')
            if no_color[-4:]=="\n\n- ": no_color = no_color[:-4]
            if no_color[-3:]=="\n\n\"": no_color = no_color[:-3]
            if no_color[-5:]=="\n\nIl ": no_color = no_color[:-5]
            if no_color[-4:]==u"\n\n\u2013 ": no_color = no_color[:-4]
            if no_color[-4:]==u"\n\n\u2014 ": no_color = no_color[:-4]
            if no_color[-4:]==u"\n\n\xab ": no_color = no_color[:-4]
            if no_color[-5:]==u"\n\n== ": no_color = no_color[:-5]
...
def get_djvu(mysite, djvuname, check_timestamp=False):
    ...
    if not os.path.exists(filename): 
        ...
        if check_timestamp:
            ...
            timestamp = time.mktime( time.strptime(date, format) )
...
if __name__ == "__main__":
    ...
    pass
    #pagename = "Le Monde gr&eacute;co-slave"
